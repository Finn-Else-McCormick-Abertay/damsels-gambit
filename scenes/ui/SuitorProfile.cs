    ...
    private bool Open {
        get; set {
            if (Open != value || (_moveTween?.IsRunning() ?? false)) {
                ...
                this.OnReady(() => {
                    _moveTween?.Kill(); _moveTween = Root.CreateTween();
                    _moveTween.TweenProperty(Root, "position", value ? OpenOffset : Highlighted ? HighlightOffset : new Vector2(), OpenDuration);
                });
            }
            field = value;
        }
    ...
    private bool Highlighted {
        get; set {
            if ((Highlighted != value || (_moveTween?.IsRunning() ?? false)) && !Open) this.OnReady(() => {
                _moveTween?.Kill(); _moveTween = Root.CreateTween();
                _moveTween.TweenProperty(Root, "position", value ? HighlightOffset : new Vector2(), HighlightDuration);
            });
            field = value;
        }
    } = false;
